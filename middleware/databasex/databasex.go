...
func InitDB(setting *configx.Database) *gorm.DB {
	...
	if err != nil {
		logx.GetLogger().Sugar().Errorf("fail to connect database: %v\n", err)
		os.Exit(-1)
	}
	sqlDb, dbErr := db.DB()
	if dbErr != nil {
...
func getDbEngineDSN(setting *configx.Database) string {
...
func KeepAlivedDb(engine *sql.DB) {
	...
	for {
		...
		err = engine.Ping()
		if err != nil {
			logx.GetLogger().Sugar().Errorf("database ping error: %v\n", err.Error())
		}
	}
}
